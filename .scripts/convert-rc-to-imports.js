...
function movePackagesAndRemoveFromMeteorPackageFile() {
	...
	Object.entries(packagesToMove).forEach(([name, packageName]) => {
		try {
			execSync(`mv packages/${ packageName } app/`);
...
function importMovedPackagesIntoMainFiles() {
	let importsFile = Object.values(packagesToMove)
		.filter((packageName) => {
			const file = fs.readFileSync(`app/${ packageName }/index.js`).toString();
			if (file.match(/Meteor\.(isClient|isServer)/g)) {
				return file.match(/Meteor\.isClient/g);
			}
			return true;
		})
		.map((packageName) => `import '/app/${ packageName }';`).join('\n');
...
import '/app/rocketchat-katex/katex.min.css';
`;
...
function fixStylelintignore() {
	...
	file += 'app/rocketchat-katex/katex.min.css\n';
...
execSync('cp node_modules/katex/dist/katex.min.css app/rocketchat-katex/');
...
`;
	...
	let file = fs.readFileSync('app/rocketchat-ui-message/client/message.js').toString();

	file = file.replace('node_modules/pdfjs-dist/build/pdf.worker.js', 'pdf.worker.min.js');

	fs.writeFileSync('app/rocketchat-ui-message/client/message.js', file);
